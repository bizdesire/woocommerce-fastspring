...
class AdminLicenseFailedEmail extends WC_Email {

    function __construct() {

        // Add email ID, title, description, heading, subject
        $this->id = 'admin_license_failed';
        $this->title = __('Admin License Failed', 'wc-fastspring');
        $this->description = __('This email is received when a license faled.', 'wc-fastspring');

        $this->heading = __('Admin LIcense Failed', 'wc-fastspring');
        $this->subject = __('[{blogname}]  Licensed Failed', 'wc-fastspring');

        // email template path
        $this->template_html = 'emails/admin-license-failed.php';
        $this->template_plain = 'emails/plain/admin-license-failed.php';
        $this->admin_email = true;
        add_action('admin_license_failed_email_notification', array($this, 'trigger'));
